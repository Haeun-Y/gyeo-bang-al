...
/*
결론적으로 말해 이런 문제는 불에 대한 BFS와 지훈이에 대한 BFS를 모두 돌림으로서 해결이 가능합니다.

먼저 불에 대한 BFS를 돌려서 미리 각 칸에 불이 전파되는 시간을 다 구해둡니다. 두 번째의 맵의 각 칸 값 = 불 전파시간을 의미합니다.

그 다음에는 지훈이에 대한 BFS를 돌리며 지훈이를 이동시킵니다.
이 때 만약 지훈이가 특정 칸을 x시간에 최초로 방문할 수 있는데 그 칸에는 x시간이나 그 이전에 불이 붙는다면 그 칸을 못가게 됩니다.
...
이렇게 BFS와 지훈이에 대한 BFS를 따로 해서 문제를 그렇게 어렵지는 않게 해결할 수 있습니다.
*/
#include <bits/stdc++.h>
using namespace std;
...
int main(void) {
	...
	while (!jihoon.empty()) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if ((dist_jihoon[curX][curY] + 1 >= dist_fire[nx][ny]) && dist_fire[nx][ny] != -1) continue;
