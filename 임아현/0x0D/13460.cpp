...
string board[11];
// dist[a][b][c][d] -> 빨간 구슬, 파란 구슬의 좌표 둘 다 저장해야 됨(동시에 움직임)
//빨간 구슬이 (a, b)이고 파란 구슬이 (c, d)에 위치한 상황에 도달하기 위한 동작의 횟수
  ...
  while (!q.empty()) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if ((n_rx == n_bx) && (n_ry == n_by)) {
        if (i == 0){ //아래로 이동한 경우 작은 게 더 늦게 출발
          if(ry < by) n_ry--;
          else n_by--;
        }
        else if (i == 1) // 오른쪽으로 이동한 경우 작은 게 더 늦게 출발
          if(rx < bx) n_rx--;
          else n_bx--;
        else if (i == 2) // 위쪽으로 이동한 경우 더 큰 게 늦게 출발
          if(ry > by)n_ry++;
          else n_by++;
        else // 왼쪽으로 이동한 경우 큰 게 더 늦게 출발
          if(rx > bx) n_rx++;
          else n_bx++;
      }
